...
function NavBar({setNavHover}: {setNavHover: (isHover: boolean) => void}){
    ...
    const liClass = (path: string) =>
        `h-15 transition-all duration-150 ease-in hover:bg-[#242424] hover:text-white ${isActive(path) ? "bg-[#242424] text-white" : "text-[#242424]"}`;

    const spanClass = `whitespace-nowrap overflow-hidden transition-all duration-150 ease-in ${isHover ? "opacity-100 max-w-40 ml-3" : "opacity-0 max-w-0"}`;

    return(
        <div className={"sidebar flex flex-col bg-[#b8b8b8] py-5 min-h-screen fixed transition-all duration-150 ease-in w-16 hover:w-48"} onMouseEnter={() => {setNavHover(true); setIsHover(true);}} onMouseLeave={() => {setNavHover(false); setIsHover(false);}}>
                <div className="logo flex cinzel-decorative-regular text-center text-xl font-thin !mt-5 px-2 justify-center items-center">
                    <span className={"title w-10/12 text-white weight-bold text-center"}><Link to="/"><img src={myLogo} alt="Logo" /></Link></span>
                </div>
                <nav className="flex flex-grow flex-col">
                    <ul className="!mt-15 font-thin">
                        <li className={liClass("/")}>
                            <Link to={"/"} className="h-full flex items-center justify-center">
                            <div className="icon flex-shrink-0 w-7 h-7 transition-all duration-150 ease-in">
                        ...
                        <li className={liClass("/hobbies")}>
                            <Link to={"/hobbies"} className="h-full flex items-center justify-center">
                                ...
                                <span className={spanClass}>Hobbies</span>
                            </Link>
                        </li>
                        <li className={liClass("/threads")}>
                            <Link to={"/threads"} className="h-full flex items-center justify-center">
                                ...
                                <span className={spanClass}>Threads</span>
                            </Link>
                        </li>
                    </ul>
                </nav>
                <div className="languageSettings w-full px-2 text-gray-800">
                    <span className={`flex ${isHover ? "flex-row gap-4" : "flex-col gap-2"} transition-all duration-150 justify-center items-center h-15`}>
                        <span className={`w-12 h-7 rounded ${language === "en" ? "bg-[#9a9a9a]" : ""} hover:bg-[#9a9a9a] transition-all flex items-center justify-center`} onClick={() => { changeLang("en"); setLanguage("en"); }}><img className="w-9 h-5" src={enFlag} alt="English" /></span>
                        <span className={`w-12 h-7 rounded ${language === "kr" ? "bg-[#9a9a9a]" : ""} hover:bg-[#9a9a9a] transition-all flex items-center justify-center`} onClick={() => { changeLang("kr"); setLanguage("kr"); }}><img className="w-10 h-5" src={krFlag} alt="Korean" /></span>
                    </span>
                </div>
        </div>
